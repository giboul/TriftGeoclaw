...
for i in range(0,AddLoop.loop_number):
        ...
        VoellmyFile.write("\n")
        VoellmyFile.write("300.0 =: snow_density\n")
        ...
        VoellmyFile.write(str(AddLoop.coulomb))
        VoellmyFile.write(" =: coulomb\n")
        VoellmyFile.close()
        print("step", str(i),"/",str(nrun),": mu = ", str(mu),", xi = ", str(xi),". Voellmy file created...")
        subprocess.run(["make",".output", "target", "CLAW="+claw,">log.txt"])
        print(f"src = {cwd+'/_output/'+nameFile+bloc+str(nsim)}")
        print(f"out ={cwd+'/runs/'+'run'+str(i)}")
